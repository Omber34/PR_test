...
namespace
{
  ...
  TEST_F(FileManagerTest, Trivial)
  {
    ...
    EXPECT_TRUE(strncmp(reinterpret_cast<const char *>(filePacket1.packets.begin()->body()), testTxtFileData,
                        ChatPacket::max_body_length) == 0);
    EXPECT_STREQ(reinterpret_cast<const char *>((++filePacket1.packets.begin())->body()), packet2p);
    EXPECT_EQ(testTxtFileDataSize - filePacket1.packets.begin()->body_length(),
              (++filePacket1.packets.begin())->body_length());
  ...
  TEST(ChatRoomModel, Trivial)
  {
    const QString roomName = "test_room";
    const QString passHash = "pass_hash";
    CreateRoom newRoom{roomName, passHash};
    ChatRoomModel roomModel;

    roomModel.CreateRoom(newRoom);
    EXPECT_EQ(roomModel.rowCount(), 1U);

    const auto roomIdx = roomModel.GetRoomByName(roomName);
    EXPECT_EQ(roomIdx.data(ChatRoomModel::RoomName).toString(), roomName);
    EXPECT_EQ(roomIdx.data(ChatRoomModel::HasPassword).toBool(), true);

    roomModel.DeleteRoom(roomName);
    EXPECT_EQ(roomModel.rowCount(), 0);
  }

  TEST(ChatUserModel, Trivial)
  {
    const QString userName = "test_user";
    CreateUser newUser{userName};
    ChatUserModel userModel;

    const auto createUser = userModel.CreateUser(newUser);
    EXPECT_EQ(userModel.rowCount(), 1U);
    EXPECT_TRUE(createUser);

    const auto userIdx = userModel.GetUserByName(userName);
    EXPECT_EQ(userIdx.data(ChatUserModel::UserId).toString(), userName);

    userModel.DeleteUser(userName);
    EXPECT_EQ(userModel.rowCount(), 0);
  }

  TEST(ChatAppTest, Trivial)
  {
    const QString userName = "test_user";
    const QString roomName = "test_room";
    const QString passHash = "pass_hash";
    ...
    auto roomModel = appModel.getRoomModel();
    auto userModel = appModel.getUserModel();

    userModel->CreateUser(newUser);
    roomModel->CreateRoom(newRoom);

    appModel.moveUserToRoom(userName, roomName);
  }
}